...
##In a standard table like Mulligan or Thompson, some of these are collapsed (i.e. ppqq is the same as qqpp, as p has no meaning in that context)
...
filenameout=filenamein+'.relateF_optimAW_'+str(max_snp)+'_'+str(FST)
...
for g in range(len(pw)):
    ...
    for gg in range(nbSNPs):
        ...
        GQ1=string.split(string.split(datum[gg][pw[g][0]],':')[GQ_key],',')  ###pulls out info field for first individual from VCF, pulls out the three precomputed genotype likelihoods
        GQ2=string.split(string.split(datum[gg][pw[g][1]],':')[GQ_key],',')  ###pulls out info field for first individual from VCF, pulls out the three precomputed genotype likelihoods
        GQ1=int(GQ1[0])
        GQ2=int(GQ2[0])
...
##        PIBSg[gg]=[PPQQg,QQPPg,PPPQg,PQPPg,PQQQg,QQPQg,PPPPg,PQPQg,QQQQg]  #AATT,TTAA,AAAT,ATAA,ATTT,TTAT,AAAA,ATAT,TTTT 
    ...
    called_SNPs=len(mask_mat)-np.sum(mask_mat)
    if called_SNPs>max_snp:
        np.clip(mask_mat+np.random.binomial(1,1-(max_snp/float(called_SNPs)),len(mask_mat)),0,1,out=mask_mat)
